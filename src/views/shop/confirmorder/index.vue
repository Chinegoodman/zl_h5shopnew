    ...
    goprevpage(){
      if (getsessionStorage("pagefrom") == "productdetails") {
        ...
        this.$router.push({
          name: "productdetails",
          params: {
            product_id: ordermass.productSkuId,
            webtype: 1
          }
        });
      }else{
            ...
            for (let bb = 0; bb < that.listshow.length; bb++) {
              if (freightdata[cc].shopId == that.listshow[bb].shopId) {
                ...
                for (let dd = 0; dd < freightdata[cc].free.length; dd++) {
                  // if (freightdata[cc].free[dd].cheep == 1) { //判断最低价的 已弃用
                    // debugger;
                    that.listshow[bb].order.freightAmount = freightdata[cc].free[dd].price;
                    that.listshow[bb].order.freightAmounttype = freightdata[cc].free[dd].type;
                    that.listshow[bb].order.freightAmountname = freightdata[cc].free[dd].name;
                    that.ajax_order[bb].order.freightAmount = freightdata[cc].free[dd].price; //初始化时设置ajax需要的运费的价格
                    // that.zongfeifn();
                  // }
                }
              }
      ...
      that.api.shopcart
        ...
        .then(data => {
          ...
          if (data.data.code == 1) {
            ...
            // 存起参数ljx  
            var lastordermsg = {
                'amont' : data.data.data.totalPrice,
                'orderId' : data.data.data.pay4Orders.join()
            }
            setsessionStorage("lastpaymsg",lastordermsg);
            that.$router.push({path:'/shop/paypage',query:{type: 'detailpage'}});
