          ...
          <div class="paydetails" v-if="fymx_item.shopId == item.shopId" v-for="(fymx_item,fymx_index) in fymx_listdata" :key="fymx_index+'title'">
            ...
            <div class="pd_list">
              ...
              <div class="li peisong" v-if="fymx_item20.type==1" v-for="(fymx_item20,fymx_index20) in fymx_item.content" :key="fymx_index20+'peisong'">
                ...
                <div class="li_price" v-if="freightstatus" @click="mbopen(item.order,item.order.freightAmounttype,item.order.freightAmountname,index);">{{fymx_item20.expressName}} ></div>
                <div class="li_price" v-if="!freightstatus" style="color:red;">请先选择顶部的收货地址 ></div>
                <div class="peisongsmall">
                  {{fymx_item20.detailName}} ￥{{fymx_item20.price}}
                </div>
              </div>
              ...
              <div class="li" v-if="fymx_item23.type==3" v-for="(fymx_item23,fymx_index23) in fymx_item.content" :key="fymx_index23+'libz'">
                ...
                <span>{{index}}</span>
                <span>{{listshow[index].order.isPackagestatus}}</span>
                ...
                <div class="checkboxwrap" @click="switchchange(index,fymx_index)">
                  <span class="radio" :class="{left:!listshow[index].order.isPackagestatus,right:listshow[index].order.isPackagestatus}">3faefw</span>
                </div>
              </div>
              ...
              <div class="li" v-if="fymx_item25.type==5" v-for="(fymx_item25,fymx_index25) in fymx_item.content" :key="fymx_index25+'lily'">
                ...
                <span></span>
                <div class="li_price"><input type="text" v-model="fymx_item25.ipt" placeholder="选填,请输入您对订单或商品的一些要求或事宜"></div>
              </div>
              <div class="li lizj" v-if="fymx_item26.type==6" v-for="(fymx_item26,fymx_index26) in fymx_item.content" :key="fymx_index26+'lizj'">
                ...
                <span></span>
                <div class="li_price">￥{{fymx_item26.price}}</div>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    switchchange(indexkey,innderindex){
      ...
      let that = this;
      this.listshow[indexkey].order.isPackagestatus = !this.listshow[indexkey].order.isPackagestatus;
      ...
      if(that.listshow[indexkey].order.isPackagestatus){
        ...
        for(let a=0;a<that.fymx_listdata[innderindex].content.length;a++){
        ...
        for(let aa=0;aa<that.fymx_listdata[innderindex].content.length;aa++){
    ...
    //总费用===>>商品费用 加 快递费运费 设置事件
    zongfeifn() {
      let feiyong = 0;
      let that = this;
      for (let i = 0; i < that.ajax_order.length; i++) {
        feiyong = feiyong + that.ajax_order[i].order.freightAmount;
        // alert(feiyong);
      }
      // 后台返回要购买的商品 总价
      let priceparamsarr = [];
      let goodsarr = [];
      for (let j = 0; j < that.listshow.length; j++) {
        for (let k = 0; k < that.listshow[j].items.length; k++) {
          goodsarr.push(that.listshow[j].items[k]);
        }
      }
      ...
      that.api.shopcart
        ...
        .then(res => {
          if (res.data.code == 1) {
            ...
            that.goodsallprice = res.data.data;
            feiyong = feiyong + this.goodsallprice;
            this.zongfei = feiyong;
          }
        })
    },
    // 运费类型选择模块==>>选择事件
    ...
    confirmorderfn() {
      ...
      axios({
        url: `${base.yxf}/wapi/v1/order/createorder`,
        method: "post",
        headers: { "Content-Type": "application/json" },
        // headers: { "Content-Type": "application/x-www-form-urlencoded" },
        dataType: "json",
        data: JSON.stringify(that.ajax_order)
      })
        .then(data => {
    ...
    //computedFreight运价计算接口     获取运价后调用 自费展示的接口方法 getshowPrice
    computedFreight() {
      // alert('收货地址接口调用！');
      let that = this;
      let ajaxdata = {
      ...
      that.api.shopcart
        ...
        .then(res => {
          ...
          setTimeout(() => {
            //有配送地址 计价相关的
            this.getshowPrice(true);
          }, 0);
        })
    },
    // 数据从存储获取   同时判断地址状态===有地址获取配送费用 无地址设置配送费为0
    pagedatashow() {
      let that = this;
      ...
      if (that.checkaddressdata()) {
        ...
        that.getshowPrice(false);
      }
    },
    // 计价相关的数据获取与展示 type:false 没有配送地址，type:true有配送地址
    getshowPrice(type) {
      let that = this;
      // console.log(that.ajax_order);
      let ajax_getshowPrice=JSON.parse(JSON.stringify(that.ajax_order));
      // {"goods":[{"skuId":1780661640987681260,"count":6}],"shopId":101763,"freight":18,"isPackage":0};
      let contentdata = [];

      for(let i=0;i<ajax_getshowPrice.length;i++){
        let goods=[];
        for(let o=0;o<ajax_getshowPrice[i].orderItemList.length;o++){
          goods.push({
            skuId:ajax_getshowPrice[i].orderItemList[o].productSkuId,
            count:ajax_getshowPrice[i].orderItemList[o].productQuantity,
          })
        }
        ...
        let freight =0;
        let isPackage =0;
        if(that.fymx_listdata.length>0){
          let freightarr = that.fymx_listdata[i].content.filter(function(item,index){
            return item.type==1;
          })
          // console.log(freightarr);
          freight = freightarr.length==0?0:freightarr[0].price;

          if(that.fymx_listdata.length>0){
            let isPackagearr = that.fymx_listdata[i].content.filter(function(item,index){
              return item.type==3;
            })
            // console.log(isPackagearr);
            if(isPackagearr.length==0){
              isPackage = 0;
            }else if(freightarr[0].price==0){
              isPackage = 0;
            }else{
              // isPackage = freightarr[0].price;
              //需要包装
              isPackage = 1;
            }
          }else{
        ...
        contentdata.push({
          shopId:ajax_getshowPrice[i].order.shopId,
          freight:freight,
          isPackage:isPackage,
          goods:goods,
        })
      ...
      this.api.shopcart
        ...
        .then(res => {
          ...
          if(res.data.code==1){
            ...
            // console.log(that.fymx_listdata);
            for(let ii=0;ii<that.fymx_listdata.length;ii++){
              that.fymx_listdata[ii].freight = that.listshow[ii].order.freightAmount;
              let res_isPackage;
              let res_isPackagearr = that.fymx_listdata[ii].content.filter(function(item,index){
                return item.type==3;
              })
              // console.log(res_isPackagearr);
              res_isPackage = res_isPackagearr[0].price==0?false:true;
              that.listshow[ii].order.isPackage = res_isPackage;
              that.listshow[ii].order.isPackagestatus = that.listshow[ii].order.isPackage;
              // console.log(res_isPackage);
            }
          }
        })
    },
