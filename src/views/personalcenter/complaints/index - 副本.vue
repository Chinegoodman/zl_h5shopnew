...
export default {
  ...
  methods: {
    ...
    complaintstype() {
      let that = this;
      that.api.personalcenter.complaintstype({}).then(res => {
    ...
    // 获取osstoken
    ossmass() {
      this.api.personalcenter.osstoken({}).then(res => {
    ...
    upimgs(e) {
      ...
      // 时间戳文件名
      const aliyunFileKey =
        new Date().getTime() + Math.floor(Math.random() * 150) + ".png";
      // OSS配置
      // let client = new OSS({
      ...
      // OSS路径
      let FileController =
        "https://zjlwhtestshimingrenzheng.oss-cn-shanghai.aliyuncs.com";
      // XMLHttpRequest 对象
