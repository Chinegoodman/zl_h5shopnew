    ...
    <div class="edit_address">
      <ul>
        <li>
          ...
          <input type="text" placeholder="请用真实姓名" v-model="addname" />
        </li>
        <li>
          <span class="title1">手机号码</span>
          <input type="text" placeholder="请输入收货人手机号码" v-model="addphone" />
        </li>
        <li class="clearfix">
        ...
        <li>
          ...
          <input type="text" placeholder="街道、小区门牌等详细地址" v-model="addaddress" />
        </li>
  ...
  export default {
    ...
    methods: {
      ...
      btn() {
        ...
        this.api.personalcenter.postaddaddress(can).then(res => {
          // console.log(res);
          if (res.data.code == 1) {
            if (adddefault == 1) {
              sessionStorage.setItem("checkaddressitem", JSON.stringify(can));
            }
            if (res.data.code == 1) {
              alert("已添加");
              that.$router.go(-1);
